...
public class PoolingHTTPClient implements HTTPClient {
	...
	public HTTPResponse fetch(HTTPRequest request) throws IOException, URISyntaxException {
		...
		// Build request
		RequestConfig reqConfig = RequestConfig.custom().setConnectTimeout(request.connectTimeout)
				.setConnectionRequestTimeout(request.requestTimeout).build();
		RequestBuilder reqBuilder = RequestBuilder.create(request.method.name()).setUri(uriBuilder.build())
				.setEntity(new StringEntity(request.body, "UTF-8")).setConfig(reqConfig);
