...
public class SmsMobileStatusPuller extends SmsBase {
	...
	private HTTPResponse pull(int type, String nationCode, String mobile, long beginTime, long endTime, int max)
			throws IOException {

		long random = SmsSenderUtil.getRandom();
		long now = SmsSenderUtil.getCurrentTime();
		JSONObject body = new JSONObject();
		body.put("sig", SmsSenderUtil.calculateSignature(this.appkey, random, now)).put("type", type).put("time", now)
				.put("max", max).put("begin_time", beginTime).put("end_time", endTime).put("nationcode", nationCode)
				.put("mobile", mobile);
		...
		HTTPRequest req = new HTTPRequest(HTTPMethod.POST, this.url).addHeader("Conetent-Type", "application/json")
				...
				.setConnectionTimeout(60 * 1000).setRequestTimeout(60 * 10000).setBody(body.toString());
	...
	public SmsStatusPullCallbackResult pullCallback(String nationCode, String mobile, long beginTime, long endTime,
			int max) throws HTTPException, JSONException, IOException {
	...
	public SmsStatusPullReplyResult pullReply(String nationCode, String mobile, long beginTime, long endTime, int max)
